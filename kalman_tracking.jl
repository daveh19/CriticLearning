...
function generate_two_reward_sequences(sequence_length = 100, noise_sigma = 0.1)
  ...
  for i = 1:sequence_length
    ...
    sequence_value[i] = sequence_id[i] - 1.5 + rand(Normal(0,1)) .* noise_sigma;
...
function kalman_update_correction(k_dict, data_row, observation_noise_model)
  ...
  K = k_dict["updated_error_covariance"] * (k_dict["updated_error_covariance"] + observation_noise_model)^(-1);
